...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, x, y, direction, image, colour):
        ...
        self.speed = 6
        if (colour==1):
            self.image = bullet_img_red
        elif (colour==2):
            self.image = bullet_img_blue
        else:
            self.image = bullet_img_green
...
class Player(pygame.sprite.Sprite):
    ...
    def get_input(self):
        ...
        if keys[pygame.K_e]:
            self.bullet_create(randint(1, 3))
        if keys[pygame.K_r]:
            self.bullet_create(1)
        if keys[pygame.K_b]:
            self.bullet_create(2)
        if keys[pygame.K_g]:
            self.bullet_create(3)
        if keys[pygame.K_d]:
            self.direction.x = 1
            self.facing_right = True
